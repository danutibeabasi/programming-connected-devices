...
Find a way to turn on and off the LED on GPIO#13 every 500ms, and the LED on GPIO#27 every 300ms.
        ...
        tim0 = Timer(0)
        tim1 = Timer(1)
        tim0.init(period = 500, mode = Timer.PERIODIC, lambda t:pin13.value(not pin13.value()))
        tim1.init(period = 300, mode = Timer.PERIODIC, lambda t:pin27.value(not pin27.value()))
